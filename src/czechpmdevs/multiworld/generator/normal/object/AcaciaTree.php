...
class AcaciaTree extends Tree {
        ...
        if ($position->getY() >= 1 && $position->getY() + $i + 1 <= 256) {
                ...
                for ($l = (int)$position->getX() - $k; $l <= $position->getX() + $k && $flag; ++$l) {
                    for ($i1 = (int)$position->getZ() - $k; $i1 <= $position->getZ() + $k && $flag; ++$i1) {
                        if ($j >= 0 && $j < 256) {
                            ...
                            /*if (!$this->canPlaceObject($world, $world->getBlockIdAt((int)$vector3->x, (int)$vector3->y, (int)$vector3->z))) {
                                $flag = false;
                            }*/
                        } else {
                            $flag = false;
            ...
            if (!$flag) {
                ...
                if (($block == BlockLegacyIds::GRASS || $block == BlockLegacyIds::DIRT) && $position->getY() < 256 - $i - 1) {
                    ...
                    $world->setBlockIdAt($position->getX(), $position->getY() - 1, $position->getZ(), BlockLegacyIds::DIRT);
                    ...
                    for ($l1 = 0; $l1 < $i; ++$l1) {
                        ...
                        $material = $world->getBlockIdAt($blockpos->getFloorX(), $blockpos->getFloorY(), $blockpos->getFloorZ());
                    ...
                    if ($face1 != $face) {
                        ...
                        for ($l4 = $l3; $l4 < $i && $k4 > 0; --$k4) {
                            if ($l4 >= 1) {
                                ...
                                $material1 = $world->getBlockIdAt($blockpos1->getFloorX(), $blockpos1->getFloorY(), $blockpos1->getFloorZ());
    ...
    private function placeLogAt(ChunkManager $worldIn, Vector3 $pos): void {
        $worldIn->setBlockIdAt((int)$pos->getX(), (int)$pos->getY(), (int)$pos->getZ(), BlockLegacyIds::WOOD2);
    }
    ...
    private function placeLeafAt(ChunkManager $worldIn, Vector3 $pos): void {
        ...
        if ($material == BlockLegacyIds::AIR || $material == BlockLegacyIds::LEAVES) {
            $worldIn->setBlockIdAt((int)$pos->getX(), (int)$pos->getY(), (int)$pos->getZ(), BlockLegacyIds::LEAVES2);
        }
    }
}
