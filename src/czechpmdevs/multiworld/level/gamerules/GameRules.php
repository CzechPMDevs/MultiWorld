...
final class GameRules {
	...
	/**
	 * If the array is empty, default GameRules will be used
	...
	public function __construct(array $gameRules = []) {
		...
		if (!isset(self::$defaultGameRules)) {
			$this->gameRules = $gameRules;
			return;
		}

		if (empty($gameRules)) {
			$this->gameRules = self::getDefaultGameRules()->getGameRules();
			return;
		}

		// Removing invalid GameRules
		foreach ($gameRules as $key => $value) {
			if (!self::$defaultGameRules->keyExists($key)) {
				unset($gameRules[$key]); // GameRule does not exist at all
			} elseif (self::getPropertyType($value) !== self::getPropertyType(self::$defaultGameRules->getGameRules()[$key])) {
				unset($gameRules[$key]); // GameRule has invalid type
			}
		}

		// Adding new GameRules
		foreach (self::$defaultGameRules->gameRules as $key => $gameRule) {
	...
	public static function getDefaultGameRules() : GameRules {
		return clone self::$defaultGameRules;
	}
	...
	public static function init(array $defaultGameRules) : void {
		self::$defaultGameRules = new GameRules($defaultGameRules);
	}
	...
	public static function loadFromWorld(World $world) : GameRules {
		...
		return self::unserializeGameRules($nbt);
	}
	...
	public static function unserializeGameRules(CompoundTag $nbt) : GameRules {
		return new GameRules(array_map(static function (StringTag $stringTag) {
	...
	public static function saveForWorld(World $world, GameRules $gameRules) : bool {
		...
		$worldData->getCompoundTag()->setTag("GameRules", self::serializeGameRules($gameRules));
		return true;
	}
	...
	public function applyToPlayer(Player $player) : void {
		$player->getNetworkSession()->sendDataPacket($this->mapGameRulesChangedPacket());
	}

	public function applyToWorld(World $world) : void {
		$pk = $this->mapGameRulesChangedPacket();
		foreach ($world->getPlayers() as $player) {
			$player->getNetworkSession()->sendDataPacket($pk);
		}
	}

	public function mapGameRulesChangedPacket() : GameRulesChangedPacket {
		$pk = new GameRulesChangedPacket();
		$pk->gameRules = array_map(static function ($gameRule) {
			if (is_float($gameRule)) {
				$rule = new FloatGameRule($gameRule, GameRules::$allowPlayersEditGameRulesFromGame);
			}
			if (is_int($gameRule)) {
				$rule = new IntGameRule($gameRule, GameRules::$allowPlayersEditGameRulesFromGame);
			}
			if (is_bool($gameRule)) {
				$rule = new BoolGameRule($gameRule, GameRules::$allowPlayersEditGameRulesFromGame);
			}
			if (!isset($rule)) {
				throw new AssumptionFailedError("GameRule object should exist");
			}
			return $rule;
		}, $this->gameRules);
		return $pk;
